...
You can include the `cex.js` file directly in your HTML:

```html
<script src="path/to/cex.js"></script>
<script>
...
```javascript
const CEXParser = require('./path/to/cex.js');
const parser = new CEXParser();
// ...
```
...
The `CEXParser` class is the main interface for parsing and interacting with CEX data. It provides methods to load data from various sources, parse it, and extract specific information.
...
console.log(parser.getUniqueBlockLabels()); // Output: ['ctsdata']
```
...
Asynchronously loads CEX data from a local File object (typically obtained from an HTML `input` element  with `type="file"`), then parses it.
...
- **Parameters**:
    - `blockLabel` (String): The label of the blocks to process (e.g., citedata, ctsdata).
